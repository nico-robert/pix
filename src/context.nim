...
proc pix_ctx_save(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_saveLayer(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_strokeSegment(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  if getListFloat(interp, objv[3], x1, y1, 
...
proc pix_ctx_strokeRect(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_quadraticCurveTo(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  # Coordinates
  var x, y, cpx, cpy: float32

  if getListFloat(interp, objv[2], cpx, cpy, 
    "wrong # args: 'coordinates1' should be {cpx cpy}") != Tcl.OK:
    return Tcl.ERROR

  if getListFloat(interp, objv[3], x, y, 
...
proc pix_ctx_arcTo(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  try:
    ctx.arcTo(x1, y1, x2, y2, objv[4].getFloat())
  except PixieError as e:
    return pixUtils.errorMSG(interp, "pix(error): " & e.msg)
...
proc pix_ctx_bezierCurveTo(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  # Coordinates
  var cp1x, cp1y, cp2x, cp2y, x, y: float32

  if getListFloat(interp, objv[2], cp1x, cp1y, 
    "wrong # args: 'coordinates1' should be {cp1x cp1y}") != Tcl.OK:
    return Tcl.ERROR

  if getListFloat(interp, objv[3], cp2x, cp2y, 
    "wrong # args: 'coordinates2' should be {cp2x cp2y}") != Tcl.OK:
    return Tcl.ERROR

  if getListFloat(interp, objv[4], x, y, 
...
proc pix_ctx_circle(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  ctx.circle(cx, cy, objv[3].getFloat())
...
proc pix_ctx_drawImage(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  if objc == 5:
    # Destination
    if getListFloat(interp, objv[3], dx, dy, 
      "wrong # args: 'destinationXY' should be {dx dy}") != Tcl.OK:
      return Tcl.ERROR

    # Size
    if getListFloat(interp, objv[4], dWidth, dHeight, 
    ...
    var sx, sy, sWidth, sHeight: float32
    # Source
    if getListFloat(interp, objv[3], sx, sy, 
      "wrong # args: 'sourceXY' should be {sx sy}") != Tcl.OK:
      return Tcl.ERROR

    # Source Size
    if getListFloat(interp, objv[4], sWidth, sHeight, 
      "wrong # args: 'sourceWH' should be {sWidth sHeight}") != Tcl.OK:
      return Tcl.ERROR

    # Destination
    if getListFloat(interp, objv[5], dx, dy, 
      "wrong # args: 'destinationXY' should be {dx dy}") != Tcl.OK:
      return Tcl.ERROR

    # Destination Size
    if getListFloat(interp, objv[6], dWidth, dHeight, 
      "wrong # args: 'destinationWH' should be {dWidth dHeight}") != Tcl.OK:
      return Tcl.ERROR
...
proc pix_ctx_strokeEllipse(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_transform(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_lineJoin(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_strokeRoundedRect(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_isPointInStroke(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_closePath(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_fontSize(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  # Context font size.
  ctx.fontSize = objv[2].getFloat()

  return Tcl.OK

proc pix_ctx_fillText(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_fillPolygon(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_strokeCircle(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
...
proc pix_ctx_setLineDash(clientData: Tcl.TClientData, interp: Tcl.PInterp, objc: cint, objv: Tcl.PPObj): cint {.cdecl.} =
  ...
  for i in 0..count-1:
    pattern.add(elements[i].getFloat())
